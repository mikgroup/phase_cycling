...
for iter = 1:niter
	...
	CEx = C * EX_xi;
	w1 = wei_func(CEx, del);
	GLn = 2*real( 1i*conj(EX_xi) .* mi .* (A'*(yi - Am0_xn)) );
	...
	while ((ak==0) || (abs(ak-ak_old)/abs(ak) > arg.thr)) && (k <= arg.subiter)
		...
		tmp1 = 1i*conj(EX_xi_dn) .* mi .* (A'*(yi-Am0_xn_dn));
		...
		if arg.btr == 1 % backtracking
			...
			while 1
				...
				fa1 = norm(yi-A * (mi.*EX_xi_dn1))^2 + beta*R1x;
				if (fa1-fa0) < 1e-8
					break
				end
				fa0 = fa1;
